...
st.title("?? Kadvi Baa Tex - Purchase Item Invoice")
st.write("Apni Purchase PDF yahan upload karein, Excel aur Tally Item Invoice XML generate karne ke liye.")
...
# ==========================================
def process_purchase_pdf(pdf_file_obj):
    text = ""
...
"""
    ...
    for index, row in df.iterrows():
        ...
        item_rate = 0
        if taxable_val > 0:
            item_rate = round(((cgst + sgst) / taxable_val) * 100)
            
        item_name = f"PURCHASE {int(item_rate)}%"
...
# ?? UI WORKFLOW
# ==========================================
uploaded_pdfs = st.file_uploader("Upload Purchase PDFs", type=["pdf"], accept_multiple_files=True)
    ...
    if st.button("Process & Generate Files", type="primary"):
        with st.spinner("Processing your PDFs... Kripya wait karein."):
            ...
            for pdf_file in uploaded_pdfs:
                try:
                    extracted_data = process_purchase_pdf(pdf_file)
                    if extracted_data:
            ...
            if all_data:
                ...
                xml_string, count = generate_purchase_xml(df.copy())
                
                st.success(f"? Success! {count} Purchase Vouchers ka Excel aur Item Invoice XML tayyar hai.")
                ...
                with col1:
                    st.download_button(
                        ...
                        data=excel_bytes, 
                        file_name="Purchase_Bills_Extracted.xlsx", 
                ...
                with col2:
                    st.download_button(
                        ...
                        data=xml_string, 
                        file_name="Tally_PurchaseItem_Import.xml", 
